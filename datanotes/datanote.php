...
<script type="text/javascript">
    ...
    var disqus_shortname = \'statedata\'; // required: replace example with your forum shortname
...
if (!$_GET['article_id']){
	if ((is_numeric($_POST['article_id'])) && (strlen($_POST['article_id']) < 5)) {
$article_id = htmlentities($_POST['article_id']);
} else { $article_id=333; }
}
else { if ((is_numeric($_GET['article_id'])) && (strlen($_GET['article_id']) < 5))
 {
...
if ( $a->pdf)
{
	$pdf = '<p><a href="pdf/'.$a->pdf.'">Download this publication in PDF</a>.</p>';
}
if ( $a->word)
{
	$word = '<p><a href="word/'.$a->word.'">Download this publication MS Word</a>.</p>';
}
$article_text = str_replace  ( '$', '&#36;', $a->article_text );

$template->add_region('content','<h2>'.$a->article_sub_title.'</h2>'.$pdf.$text.$article_text.$comments.$footer);
//write page
