...
$template->add_region('content','
	<?php
		...
		$arVariableValues = $sample->getActivityVariableArray ($variable, $region, $provider);
		$arColumnNames = $sample->getActivityColumnNamesArray ($variable);
		$axis = $sample->getAxisLabel($variable);
		$legend = $sample->getLegendName($variable, $region, $provider);
		...
		if ($print == 0) {
			echo "<br><a target=\'_new\'  href=\'activity_2.php?print=1&region=$region&provider=$provider&variable=$variable\' >Printer-Friendly Format</a>";
		}
		echo "<br> $axis_note";
