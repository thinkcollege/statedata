...
var $qls;
	...
	$username = $this->qls->Security->make_safe($_POST['username']);
		...
		$user_info = $this->fetch_user_info($username);
			...
			if (substr($this->qls->config['cookie_domain'], 0, 1) == '.') {
				...
				$change_link = "http://www{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}users/change_password.php?code={$code}";
				}
				else {
				$change_link = "http://www{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/users/change_password.php?code={$code}";
				}
				...
				if (substr($this->qls->config['cookie_path'], -1) == '/') {
				$change_link = "http://{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/users/change_password.php?code={$code}";
				}
				else {
				$change_link = "http://{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/users/change_password.php?code={$code}";
				}
	...
	function fetch_user_provider_info($row) {
		if ($row['provider_id'] != 0) {
			$result = $this->qls->SQL->query('select * from dmr_providers where provider_id ='. $row['provider_id'] );
