...
class qls {
	...
	$this->main_directory = str_replace('/includes_md', '', dirname(__FILE__));
	...
	function generate_activation_link($generated_code, $username) {
		...
		if (substr($this->config['cookie_domain'], 0, 1) == '.') {
			...
			$activation_link = "http://www{$this->config['cookie_domain']}{$this->config['cookie_path']}users/activate_md.php?code={$generated_code}&username={$username}";
			}
			else {
			$activation_link = "http://www{$this->config['cookie_domain']}{$this->config['cookie_path']}/users/activate_md.php?code={$generated_code}&username={$username}";
			}
			...
			// Does it have a / at the end?
			if (substr($this->config['cookie_path'], -1) == '/') {
			$activation_link = "http://{$this->config['cookie_domain']}{$this->config['cookie_path']}users/activate_md.php?code={$generated_code}&username={$username}";
			}
			else {
			$activation_link = "http://{$this->config['cookie_domain']}{$this->config['cookie_path']}/users/activate_md.php?code={$generated_code}&username={$username}";
			}
