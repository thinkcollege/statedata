...
var $qls;
	...
	$username = $this->qls->Security->make_safe($_POST['username']);
		...
		$user_info = $this->fetch_user_info($username);
			...
			if (substr($this->qls->config['cookie_domain'], 0, 1) == '.') {
				...
				$change_link = "http://www{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}users/change_password_md.php?code={$code}";
				}
				else {
				$change_link = "http://www{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/users/change_password_md.php?code={$code}";
				}
				...
				if (substr($this->qls->config['cookie_path'], -1) == '/') {
				$change_link = "http://{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}users/change_password_md.php?code={$code}";
				}
				else {
				$change_link = "http://{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/change_password_md.php?code={$code}";
				}
			}
	...
	function fetch_user_info($username) {
		...
		$row = $this->qls->SQL->fetch_array($result);
		return $row;
		//return $this->fetch_user_provider_info($row);
		}
		else {
		return false;
		}
	}
	function fetch_user_provider_info($row) {
		if ($row['provider_id'] != 0) {
			$result = $this->qls->SQL->query('select * from mdda_providers where provider_id ='. $row['provider_id'] );
			$providerinfo = $this->qls->SQL->fetch_array($result);
			echo $providerinfo['vendor'];
			$this->merge ($row,$providerinfo);
	 ...
	 * @return void but will output error if found
	 */
	function insert_registration_data($username, $password, $email, $providerId = "") {
	...
	$values = array(
		...
		// Email stuff...
			if (substr($this->qls->config['cookie_domain'], 0, 1) == '.') {
				if (substr($this->qls->config['cookie_path'], -1) == '/') {
				$activation_link = "http://www{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}users/activate.php?code={$generated_code}&username={$username}";
				}
				else {
				$activation_link = "http://www{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/users/activate.php?code={$generated_code}&username={$username}";
				}
				...
				if (substr($this->qls->config['cookie_path'], -1) == '/') {
				$activation_link = "http://{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}users/activate.php?code={$generated_code}&username={$username}";
				}
				else {
				$activation_link = "http://{$this->qls->config['cookie_domain']}{$this->qls->config['cookie_path']}/users/activate.php?code={$generated_code}&username={$username}";
				}
