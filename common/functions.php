...
function getStatesFiltered($element_name, $dataset, $showAll = 1) { 
	...
	if ($dataset == "agency_mrdd"){
		$html	.= " onchange='updateTrendVariables(this,\"".$dataset."\");' ";
	}
	$html	.= ">";
 	$html	. "<option value=0>Choose state</option>";
...
function get_vr_tables($agency){
	$database = Database::getDatabase();
	$results = $database->query("SHOW TABLE STATUS LIKE 'vr_" . ($agency == 'VR' ? 'rsa%\'' : 'rsa%\''));
	print "<!-- query:SHOW TABLE STATUS LIKE 'vr_" . ($agency == 'VR' ? 'rsa%\'' : 'rsa%\'') . " agency:$agency -->";
	$html = "<select id=vr_table name=vr_table >";
...
function get_individual_tables(){
	...
	*/
	function translateState($st) {
		...
		$rs = $db->query("SELECT $col FROM `states` WHERE $where = '$st'");
...
function get_vr_table_columns_as_checkboxes($table, $total_vars) {
	...
	$results = $database->query("SHOW TABLE STATUS LIKE 'vr_rsa%'");

	$vr_table = $database->fetch_result($results, 0,'Name');
	$fields = $database->query("SELECT  * FROM `labels` where `table_name` = '$vr_table' order by `sort_order`, `short_name`");
	$html = '';
	for ($result = 0;$result < $database->num_rows($fields); $result++){
...
function get_acs_table_columns_as_checkboxes($table, $total_vars) {
	...
	$fields = $database->query("SELECT  * FROM `labels` where `table_name` = '$acs_table' order by `sort_order`, `short_name`");
	$html = '';
	for ($result = 0;$result < $database->num_rows($fields); $result++){
		$field = $database->fetch_result($fields,$result,"column_name");
		$description = $database->fetch_result($fields,$result,"description");
	...
	function get_table_columns_as_radio($table, $total_vars) {
		...
		for ($result = 0; $result < $database->num_rows($fields); $result++) {
			...
			&& $field != "YEAR" && $field != "id") {
				...
				//now write the checkbox
				$html .= "<p class=\"$className\"><input id=\"c$result\" type=\"radio\" name=\"var\" value=\"$field\"> <label for=\"c$result\"><strong>$short_name</strong></label></p> <blockquote>$desc</blockquote></p>";
			}
		}
		return $html;
	}
