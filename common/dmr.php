...
function getRegions($element_name, $showAll = 1, $provider = 'ALL', $regiontype='normal', $provider_id = 0, $year = 'All') {
	...
	if ($provider == 'ALL'){
		$results = $database->query("Select distinct region from spec_dmr6 where region != 'x' and region is not null and region != '' order by region");
	} else {
		if ($provider_id == 0) {
			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider' and region is not null and region != '' order by region");
		} else {
			$results = $database->query("Select distinct region from spec_dmr6 where vendor_id = $provider_id  and region is not null and region != '' order by region");
		}
	}
	...
	if ($_SERVER['PHP_AUTH_USER'] =='dmruser') {
		if ($regiontype=='normal') {
			...
			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider'  and region is not null and region != '' and CRS_contract in ( " . $this->getContractNumbers ('Berkshire') . ") ");
			if ( $database->num_rows($results) > 0 ) {
				$html= $html . "<option value='x_Berkshire'>Berkshire</option>\n";
			}

			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider' and region is not null and region != '' and CRS_contract in ( " . $this->getContractNumbers ('Franklin/Hampshire') . ") ");
			if ( $database->num_rows($results) > 0 ) {
				$html= $html . "<option value='x_Franklin/Hampshire'>Franklin/Hampshire</option>\n";
			}

			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider' and region is not null and region != '' and CRS_contract in ( " . $this->getContractNumbers ('Springfield/Westfield') . ") ");
			if ( $database->num_rows($results) > 0 ) {
				$html= $html . "<option value='x_Springfield/Westfield'>Springfield/Westfield</option>\n";
			}

			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider' and region is not null and region != '' and CRS_contract in ( " . $this->getContractNumbers ('Holyoke/Chicopee') . ") ");
			if ( $database->num_rows($results) > 0 ) {
				$html= $html . "<option value='x_Holyoke/Chicopee'>Holyoke/Chicopee</option>\n";
			}
			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider' and region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '". $this->getContractNumbers ('West') ."'  ");

			if ( $database->num_rows($results) > 0 ) {
				$html= $html . "<option value='x_West'>West</option>\n";
			}
			$results = $database->query("Select distinct region from spec_dmr6 where vendor = '$provider' and region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '". $this->getContractNumbers ('Central') ."' ");
			if ( $database->num_rows($results) > 0 ) {
				$html= $html . "<option value='x_Central'>Central</option>\n";
			}
		}
	...
	if ($region == 'ALL' || $region=='0') {
		...
		if (substr_count($region, "x_") > 0 ) {
			...
			if ($region =='Central' || $region == 'West') {
				$query = "Select distinct vendor, vendor_id from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '". $this->getContractNumbers ($region) ."' order by vendor";
			} else {
				$query = "Select distinct vendor, vendor_id from spec_dmr6 where region is not null and region != '' and  in (". $this->getContractNumbers($region) .") order by vendor";
			}
		} else {
			$query = "Select distinct vendor, vendor_id from spec_dmr6 where region='". $region ."' and region is not null and region != '' order by vendor";
		}
	}
...
function getProviderName($vendor_id) {
	...
	$results = $database->query("Select distinct vendor from spec_dmr6 where region is not null and region != '' and vendor_id = " . $vendor_id );
	for ($result=0;$result< $database->num_rows($results);$result++){
...
function getRegionVariableArray ($variable, $year) {
	...
	for($i=0; $i<count($arRegions); $i++) {
		...
		if ($variable == 'Num_HrsInd') {
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsInd is not null and HrsInd != 0" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsInd is not null and HrsInd != 0" . $whereClause;
				}
			} else {
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsGroup is not null and HrsGroup != 0" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsGroup is not null and HrsGroup != 0" . $whereClause;
				}
			} else {
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsFac is not null and HrsFac != 0" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsFac is not null and HrsFac != 0" . $whereClause;
				}
			} else {
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsTransition is not null and HrsTransition != 0" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsTransition is not null and HrsTransition != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and OthrNonpaid is not null and OthrNonpaid != 0" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and OthrNonpaid is not null and OthrNonpaid != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' " . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") " . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(HrsInd) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsInd is not null and HrsInd != 0" . $whereClause;
				} else {
					$query = "Select avg(HrsInd) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsInd is not null and HrsInd != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				...
				if ($useIn == false) {
					$query = "Select avg(hrsGroup) as total from spec_dmr6 region is not null and region != '' and where LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and hrsGroup is not null and hrsGroup != 0" . $whereClause;
				} else {
					$query = "Select avg(hrsGroup) as total from spec_dmr6 region is not null and region != '' and where CRS_Contract in (" . $contractNumbers . ") and hrsGroup is not null and hrsGroup != 0" . $whereClause;
				}
			} else {
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(HrsFac) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsFac is not null and HrsFac != 0" . $whereClause;
				} else {
					$query = "Select avg(HrsFac) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsFac is not null and HrsFac != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(HrsVolunteer) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
				} else {
					$query = "Select avg(HrsVolunteer) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(HrsTransition) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsTransition is not null and HrsTransition != 0" . $whereClause;
				} else {
					$query = "Select avg(HrsTransition) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsTransition is not null and HrsTransition != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(OthrNonpaid) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and OthrNonpaid is not null and OthrNonpaid != 0" . $whereClause;
				} else {
					$query = "Select avg(OthrNonpaid) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and OthrNonpaid is not null and OthrNonpaid != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(dol_ind/HrsInd) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsInd is not null and HrsInd != 0 and dol_ind is not null and dol_ind != 0" . $whereClause;
				} else {
					$query = "Select avg(dol_ind/HrsInd) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsInd is not null and HrsInd != 0 and dol_ind is not null and dol_ind != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(dol_group/hrsGroup) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and hrsGroup is not null and hrsGroup != 0 and dol_group is not null and dol_group != 0" . $whereClause;
				} else {
					$query = "Select avg(dol_group/hrsGroup) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and hrsGroup is not null and hrsGroup != 0 and dol_group is not null and dol_group != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select  avg(dol_facility/HrsFac) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsFac is not null and HrsFac != 0 and dol_facility is not null and dol_facility != 0" . $whereClause;
				} else {
					$query = "Select  avg(dol_facility/HrsFac) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsFac is not null and HrsFac != 0 and dol_facility is not null and dol_facility != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(HrsInd) as part from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsInd is not null and HrsInd != 0" . $whereClause;
				} else {
					$query = "Select sum(HrsInd) as part from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsInd is not null and HrsInd != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(totalHours) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' " . $whereClause;
				} else {
					$query = "Select sum(totalHours) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") " . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(HrsGroup) as part from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsGroup is not null and HrsGroup != 0" . $whereClause;
				} else {
					$query = "Select sum(HrsGroup) as part from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsGroup is not null and HrsGroup != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(HrsFac) as part from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsFac is not null and HrsFac != 0" . $whereClause;
				} else {
					$query = "Select sum(HrsFac) as part from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsFac is not null and HrsFac != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(totalHours) as total from spec_dmr6 where region is not null and region != '' and ( CRS_Contract, 1 ) = '" . $contractNumbers . "' " . $whereClause;
				} else {
					$query = "Select sum(totalHours) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") " . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(HrsVolunteer) as part from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
				} else {
					$query = "Select sum(HrsVolunteer) as part from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(HrsTransition) as part from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and HrsTransition is not null and HrsTransition != 0" . $whereClause;
				} else {
					$query = "Select sum(HrsTransition) as part from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and HrsTransition is not null and HrsTransition != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select sum(OthrNonpaid) as part from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and OthrNonpaid is not null and OthrNonpaid != 0" . $whereClause;
				} else {
					$query = "Select sum(OthrNonpaid) as part from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and OthrNonpaid is not null and OthrNonpaid != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(dol_ind) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and dol_ind is not null  and dol_ind != 0" . $whereClause;
				} else {
					$query = "Select avg(dol_ind) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and dol_ind is not null  and dol_ind != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(dol_group) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and dol_group is not null  and dol_group != 0" . $whereClause;
				} else {
					$query = "Select avg(dol_group) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and dol_group is not null  and dol_group != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select avg(dol_Facility) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and dol_Facility is not null  and dol_Facility != 0" . $whereClause;
				} else {
					$query = "Select avg(dol_Facility) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and dol_Facility is not null  and dol_Facility != 0" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as ys from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and trim(YNInd) = 'Y'" . $whereClause;
				} else {
					$query = "Select count(*) as ys from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and trim(YNInd) = 'Y'" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and ( trim(YNInd) in ('Y','N')) " . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and ( trim(YNInd) in ('Y','N')) " . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as ys from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and trim(YNGroup) = 'Y'" . $whereClause;
				} else {
					$query = "Select count(*) as ys from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and trim(YNGroup) = 'Y'" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and ( trim(YNGroup) in ('Y','N')) " . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and ( trim(YNGroup) in ('Y','N')) " . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as ys from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and trim(YNFacility) = 'Y'" . $whereClause;
				} else {
					$query = "Select count(*) as ys from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and trim(YNFacility) = 'Y'" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and ( trim(YNFacility) in ('Y','N'))" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and ( trim(YNFacility) in ('Y','N'))" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and trim(IndSupEmp) = 'Y'" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and trim(IndSupEmp) = 'Y'" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and trim(GroupSupEmp) = 'Y'" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and trim(GroupSupEmp) = 'Y'" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and ( trim(IndSupEmp) in ('Y','N'))" . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and ( trim(IndSupEmp) in ('Y','N'))" . $whereClause;
				}
			...
			if (substr_count($arRegions[$i], "x_") > 0 ) {
				if ($useIn == false) {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '" . $contractNumbers . "' and ( trim(GroupSupEmp) in ('Y', 'N')) " . $whereClause;
				} else {
					$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and CRS_Contract in (" . $contractNumbers . ") and ( trim(GroupSupEmp) in ('Y', 'N')) " . $whereClause;
				}
...
function getActivityVariableArray ($variableType, $region, $provider, $year = '04', $vendor_id = 0) {
	...
	$whereClause = " and reporting_period='20". $year ."' ";
	...
	$results = $database->query("Select count(*) as total from spec_dmr6 where 1=1  and region is not null and region != '' " . $whereClause );
	...
	if ($variableType == 'numberinactivity') {
		for($i=0; $i<count($columArray); $i++) {
			$query = "Select count(*) as total from spec_dmr6 where " . $columArray[$i] . " is not null  and region is not null and region != '' and ". $columArray[$i] . " !=0 ". $whereClause ;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
	} elseif ($variableType == 'percent') {
		for($i=0; $i<count($columArray); $i++) {
			$results = $database->query("Select count(*) as total from spec_dmr6 where " . $columArray[$i] . " is not null and region is not null and region != '' and ". $columArray[$i] . " !=0  ". $whereClause);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columArray); $i++) {
			$query = "Select sum(" . $columArray[$i] . ") as total from spec_dmr6 where " . $columArray[$i] . " is not null and region is not null and region != '' and ". $columArray[$i] . " !=0  " . $whereClause;
//echo $query ." <BR>";
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columArray); $i++) {
			...
			$query = "SELECT AVG(" . $columArray[$i] . ") AS mean FROM spec_dmr6 where  " . $columArray[$i] . " is not null and region is not null and region != '' and ". $columArray[$i] . " !=0  " . $whereClause;
			$results = $database->query($query);
...
// totalwages, ALL, ALL, 04
		...
		$query = "Select sum(" . $columWagesArray[$i] . ") as total from spec_dmr6 where " . $columWagesArray[$i] . " is not null and region is not null and region != '' and ". $columWagesArray[$i] . " !=0  ". $whereClause;
	//	echo $query ."<BR>";
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$results = $database->query("Select avg(" . $columWagesArray[$i] . ") as total from spec_dmr6 where " . $columWagesArray[$i] . " is not null and region is not null and region != '' and ". $columWagesArray[$i] . " !=0  " . $whereClause);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$query = "Select count(" . $columWagesArray[$i] . ") as total from spec_dmr6 where  region is not null and region != '' and trim(" . $columWagesArray[$i] . ") = 'Y' ". $whereClause;
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$query = "Select count(*) as total from spec_dmr6 where   region is not null and region != '' and  trim( " . $columWagesArray[$i] . " ) IN ('N', 'Y') ". $whereClause;
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			...
			$query = "Select avg(" . $columWagesArray[$i] . "/" . $columHoursArray[$i] . ") as total from spec_dmr6 where " . $columWagesArray[$i] . " is not null and ". $columWagesArray[$i] . " !=0  and " . $columHoursArray[$i] . " is not null  and region is not null and region != '' and ". $columHoursArray[$i] . " !=0  " . $whereClause;
			//echo $query;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columEmpArray); $i++) {
			$query = "Select count(" . $columEmpArray[$i] . ") as total from spec_dmr6 where  region is not null and region != '' and trim(" . $columEmpArray[$i] . ") = 'Y' ". $whereClause;
...
//echo $returnArray[$i] ."<br>";
		...
		if ($year != '09' && $year != '10') {
			...
			//for($i=0; $i<2; $i++) {
				...
				print "\n\n<!-- where:$whereClause -->";
				$query = "SELECT count(*) as total FROM `spec_dmr6` WHERE  region is not null and region != '' and  HrsInd > 0 or dol_ind > 0 " . $whereClause;
				$results = $database->query($query);
				$totalRecords1 = $database->fetch_result($results,$result,'total');

				$query = "SELECT count( * ) AS total FROM `spec_dmr6` WHERE  region is not null and region != '' and IndSupEmp = 'Y' AND ( HrsInd =0 OR dol_ind =0 )" . $whereClause;
				$results = $database->query($query);
				$totalRecords2 = $database->fetch_result($results,$result,'total');
				
				$totalRecords = $totalRecords1 + $totalRecords2;
				
				$query = "Select count(IndSupEmp) as total from spec_dmr6 where  region is not null and region != '' and IndSupEmp = 'Y' ". $whereClause;
				$results = $database->query($query);
				...
				if ($totalRecords == 0 ) {
					...
					$returnArray[0] = ($Y/$totalRecords) * 100;
				}
				
				$query = "SELECT count(*) as total FROM `spec_dmr6` WHERE  region is not null and region != '' and  (HrsGroup > 0 or dol_Group > 0) " . $whereClause;
				$results = $database->query($query);
				$totalRecords1 = $database->fetch_result($results,$result,'total');

				$query = "SELECT count( * ) AS total FROM `spec_dmr6` WHERE  region is not null and region != '' and GroupSupEmp = 'Y' AND ( HrsGroup =0 OR dol_Group =0 )" . $whereClause;
				$results = $database->query($query);
				$totalRecords2 = $database->fetch_result($results,$result,'total');
				
				$totalRecords = $totalRecords1 + $totalRecords2;
				
				$query = "Select count(GroupSupEmp) as total from spec_dmr6 where  region is not null and region != '' and GroupSupEmp = 'Y' ". $whereClause;
				$results = $database->query($query);
				...
				if ($totalRecords == 0 ) {
					...
					$returnArray[1] = ($Y/$totalRecords) * 100;
				}
			//}
			//for($i=0; $i<count($returnArray); $i++) {
			//	$returnArray[$i] = ($returnArray[$i]/$totalRecords) * 100;
			...
			$query = "SELECT count(*) as total FROM `spec_dmr6` WHERE  region is not null and region != '' and  IndSupEmp  = 'Y' "  . $whereClause;
			$results = $database->query($query);
			$totalRecords1 = $database->fetch_result($results,$result,'total');
			
			$query = "SELECT count(*) as total FROM `spec_dmr6` WHERE  region is not null and region != '' and  Indv_intg_empl_participate  = 0 "  . $whereClause;
			$results = $database->query($query);
			$totalRecords2 = $database->fetch_result($results,$result,'total');
			$returnArray[0] = ($totalRecords1/$totalRecords2) * 100;

			$query = "SELECT count(*) as total FROM `spec_dmr6` WHERE  region is not null and region != '' and  GroupSupEmp  = 'Y' "  . $whereClause;
		//	echo $query;
			$results = $database->query($query);
			$totalRecords1 = $database->fetch_result($results,$result,'total');
			...
			$results = $database->query($query);
			$totalRecords2 = $database->fetch_result($results,$result,'total');
			$returnArray[1] = ($totalRecords1/$totalRecords2) * 100;	
	...
	if ($region == "ALL") {
		...
		if (substr_count($region, "x_") > 0 ) {
			...
			if ($region == 'West' || $region == 'Central') {
				$useIn = false;
				$query = "Select distinct vendor from spec_dmr6 where  region is not null and region != '' and LEFT( CRS_Contract, 1 ) = '". $this->getContractNumbers($region) ."' order by vendor";
			} else {
				$query = "Select distinct vendor from spec_dmr6 where  region is not null and region != '' and CRS_Contract in (". $this->getContractNumbers($region) .") order by vendor";
			}
		} else {
...
//		echo "15<br>";
	...
	//total servedd
	$query = "Select count(*) as total from spec_dmr6 where   region is not null and region != '' and " . $provderWhere . " " . $region;
//	echo $query ."<br>";
	$results = $database->query($query);
	$total = $database->fetch_result($results,$result,'total');
	echo "<td>&nbsp;" .$total ."</td>\n";
	if ($year !="ALL" && $year !="04" && $year !="05" ) {
		...
		$query = "Select count(*) as total from spec_dmr6 where  region is not null and region != '' and  " . $provderWhere . " " . $region . " and NewIndJob='Y'";
		//	echo $query ."<br>";
		$results = $database->query($query);
	...
	if ($report == "number") {
		...
		$query = "Select count(*) as total from spec_dmr6 where   region is not null and region != '' and " . $provderWhere . " and HrsInd is not null and HrsInd != 0" . $region;
		$results = $database->query($query);
		$HrsInd = $database->fetch_result($results,$result,'total');
		echo "<td>&nbsp;" . $HrsInd ."</td>\n";

		$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and HrsGroup is not null and HrsGroup != 0" . $region;
		$results = $database->query($query);
		$HrsGroup = $database->fetch_result($results,$result,'total');
		echo "<td>&nbsp;" . $HrsGroup ."</td>\n";

		$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and HrsFac is not null and HrsFac != 0" . $region;
		$results = $database->query($query);
		$HrsFac = $database->fetch_result($results,$result,'total');
		echo "<td>&nbsp;" . $HrsFac ."</td>\n";

		$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and (HrsVolunteer > 0  )" . $region;
		$results = $database->query($query);
		$HrsVolunteer = $database->fetch_result($results,$result,'total');
		echo "<td>&nbsp;" . $HrsVolunteer ."</td>\n";

		$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and  " . $provderWhere . " and HrsTransition is not null and HrsTransition != 0" . $region;
		$results = $database->query($query);
		...
		{
			$query = "Select count(*) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and OthrNonpaid is not null and OthrNonpaid != 0" . $region;
			$results = $database->query($query);
		...
		for($i=0; $i<count($columArray); $i++) {
			...
			$query = "SELECT AVG(" . $columArray[$i] . ") AS mean FROM spec_dmr6 where region is not null and region != '' and " . $provderWhere . "  and (" . $columArray[$i] . " > 0 ) " . $region;
			//echo "<td>&nbsp;" .$query ."</td>";
			$results = $database->query($query);
			echo "<td>&nbsp;" . number_format($database->fetch_result($results,$result,'mean'), 1, '.', '') . "</td>\n";
		}
		for($i=0; $i<count($columArray); $i++) {
			$query = "Select sum(" . $columArray[$i] . ") as part from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and " . $columArray[$i] . " > 0" . $region;

			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');

			$query = "Select sum(totalHours) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . "  and (" . $columArray[$i] . " > 0 or " . $columArray[$i] . " = 0) " . $region;
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$results = $database->query("Select avg(" . $columWagesArray[$i] . ") as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and " . $columWagesArray[$i] . " is not null and ". $columWagesArray[$i] . " !=0  " . $region);
		...
		for($i=0; $i<count($columWagesOverArray); $i++) {
			$results = $database->query("Select count(*) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and trim(" . $columWagesOverArray[$i] . ") in ('N','Y') " . $region);
			$totalRecords =  $database->fetch_result($results,$result,'total') ;

			$results = $database->query("Select count(*) as total from spec_dmr6 where region is not null and region != '' and " . $provderWhere . " and trim(" . $columWagesOverArray[$i] . ") ='Y'" . $region);
