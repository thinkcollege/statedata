...
*/
	...
	static function getFilterValues() {
		return array('year' => !empty($_REQUEST['y']) ? $_REQUEST['y'] : '',
			...
			'areaOffice' => !empty($_REQUEST['ao']) ? mysql_real_escape_string($_REQUEST['ao']) : '',
...
*/
	static function getActivityColumnNamesArray ($variable) {
		switch ($variable) {
			...
			case 'percent2':
				return array('Individual Competitive Job', 'Individual Contracted Job','Group Integrated Job','Self Employment', 'Facility Based/Sheltered Job');
        ...
        case 'setaside':	return array('Individual Contracted Job','Group Integrated Job', 'Facility Based/Sheltered Job');
            ...
            case 'lowSelfemp':
            case 'highSelfemp':
            return array('Hours worked in self employment', 'Gross business income','Gross business expenses');
			default:
				return array('Individual Competitive Job', 'Individual Contracted Job','Group Integrated Job','Self Employment', 'Facility Based/Sheltered Job','Community based non work','Facility based non work');
		}
	}
	...
	static function getLegendName($variable, $region, $provider, $year) {
       ...
       $rep_period = self::getReportingDates($f['year']);
		$legend	= ($f['provider'] ? " for " . self::getProviderName($f['provider']) : " for all providers ")
				...
				. ($f['region'] && $f['region'] != 'all' ? " in the {$f['region']} region" : " in all regions") . " during the reporting period, $rep_period"
...
*/
	static function getActivityVariableArray($type) {
		...
		switch ($type) {
			case 'totalwages': 				$vars = array('dol_indComp','dol_indCont', 'dol_GroupInt','dol_selfEmp', 'dol_Facility');	break;
			case 'meanwage':				$vars = array('dol_indComp','dol_indCont', 'dol_GroupInt','dol_selfEmp', 'dol_Facility'); break;
			...
			case 'meanhourlywage':			$vars = array('HrsIndComp', 'dol_indComp','HrsIndCont', 'dol_indCont', 'HrsGroupInt', 'dol_GroupInt','HrsSelfEmp', 'dol_selfEmp', 'hrsFac', 'dol_Facility'); break;
         ...
         case 'totalhours':
         	...
         	$vars = array('HrsIndComp','HrsIndCont', 'HrsGroupInt','HrsSelfEmp','HrsFac');	break;
         
         case 'avghours':	$vars = array('HrsIndComp','HrsIndCont', 'HrsGroupInt','HrsSelfEmp','HrsFac');	break;
          case 'meanSelfemp':
          case 'totalSelfemp':
          case 'lowSelfemp':
          case 'highSelfemp':
          	$vars = array('HrsSelfEmp','dol_selfEmp', 'expens_selfEmp');	break;
          case 'setaside':	$vars = array('HrsIndCont', 'HrsGroupInt','HrsFac');	break;
           ...
           case 'selfEmpperc':	$vars = array('HrsSelfEmp');	break;
			default: 						$vars = array('HrsIndComp','HrsIndCont', 'HrsGroupInt','HrsSelfEmp', 'HrsFac', 'HrsComNonWork', 'HrsFacNonWork');
			
		}
		 if ($type == 'paidtimeoff') {
			$cols	.= "FORMAT(SUM(IFNULL(`YN_pto`,0) & " . self::FLAG_YN_PTO_IN_COMP . " = " . self::FLAG_YN_PTO_IN_COMP . ")  / SUM(`HrsIndComp` > 0 OR `dol_indComp` > 0) * 100,2) AS `{$labels[0]}`"
         . "FORMAT(SUM(IFNULL(`YN_pto`,0) & " . self::FLAG_YN_PTO_IN_CONT . " = " . self::FLAG_YN_PTO_IN_CONT . ")  / SUM(`HrsIndCont` > 0 OR `dol_indCont` > 0) * 100,2) AS `{$labels[1]}`"
					. "FORMAT(SUM(IFNULL(`YN_pto`,0) & " . self::FLAG_YN_PTO_GRP_INT . " = " . self::FLAG_YN_PTO_GRP_INT . ")  / SUM(`HrsGroupInt` > 0 OR `dol_GroupInt` > 0) * 100,2) AS `{$labels[2]}`"
					. "FORMAT(SUM(IFNULL(`YN_pto`,0) & " . self::FLAG_YN_PTO_FAC . " = " . self::FLAG_YN_PTO_FAC . ")  / SUM(`HrsFac` > 0 OR `dol_Facility` > 0) * 100,2) AS `{$labels[3]}`";
		} 
      
 else if ($type == 'setaside') {
		
        $cols	.= "FORMAT(SUM(IFNULL(`YN_sa`,0) & " . self::FLAG_YN_SETA_IN_CONT . " = " . self::FLAG_YN_SETA_IN_CONT . ")  / SUM(`HrsIndCont` > 0 OR `dol_indCont` > 0) * 100,2) AS `{$labels[0]}`"
					. "FORMAT(SUM(IFNULL(`YN_sa`,0) & " . self::FLAG_YN_SETA_GRP_INT . " = " . self::FLAG_YN_SETA_GRP_INT . ")  / SUM(`HrsGroupInt` > 0 OR `dol_GroupInt` > 0) * 100,2) AS `{$labels[1]}`"
					. "FORMAT(SUM(IFNULL(`YN_sa`,0) & " . self::FLAG_YN_SETA_FAC . " = " . self::FLAG_YN_SETA_FAC . ")  / SUM(`HrsFac` > 0 OR `dol_Facility` > 0) * 100, 2) AS `{$labels[2]}`";
		} 
			...
			for ($i = 0; $i < count($vars); $i++) {
               ...
               case 'selfEmpnum':
               	$cols .= "SUM((IFNULL(`$col`, 0) > 0)) AS `{$labels[$i]}`";		break;
					...
					case 'percent2':	
               case 'selfEmpperc':
               				$cols .= "FORMAT((SUM((IFNULL(`$col`, 0) > 0)) / COUNT(1)) * 100, 2) AS `{$labels[$i]}`"; break;
					case 'totalhours':
               case 'totalSelfemp':
               ...
               case 'avghours':				$cols .= "FORMAT(SUM(IF(`$col` AND `$col` > 0 AND 1, `$col`, 0)) / SUM(IF(`$col` AND `$col` > 0 AND 1, 1, 0)),2) AS `{$labels[$i]}`";	break;
               
					case 'numberemployed10of12': 	$cols .= "SUM(`$col` = 'Y') AS `{$labels[$i]}`";	break;
               case 'numover20':				$cols .= "FORMAT(SUM(IF(`$col` AND 1 AND ($col / 4) > 20, 1,0)), 0) AS `{$labels[$i]}`";	break;

               case 'percover20':				$cols .= "FORMAT(SUM(IF(`$col` AND 1 AND ($col / 4) > 20, 1,0)) / SUM((IFNULL(`$col`, 0) > 0)) * 100, 1) AS `{$labels[$i]}`";	break;
                 case 'percvol':				$cols .= "FORMAT(SUM(IF(`$col` AND $col > 0 AND 1 AND `YN_vol` = 'Y'  , 1,0)) / SUM((IFNULL(`$col`, 0) > 0)) * 100, 1) AS `{$labels[$i]}`";	break;
                 
                  case 'lowSelfemp':				$cols .= "FORMAT(MIN(IF(`$col` IS NOT NULL , $col,NULL)),2) AS `{$labels[$i]}`";	break;
                  case 'highSelfemp':				$cols .= "FORMAT(MAX(IF(`$col` IS NOT NULL, $col, NULL)),2) AS `{$labels[$i]}`"; break;
				}
				$sep = $cols ? ",\n" : '';
			}
		}
	...
	static function getActivityTotalArray($type) {
		...
		$labels = 'Totals';
		$f		= self::getFilterValues();
		$where = self::getFilterClause('activity') . " AND (`HrsIndComp` >0 or `HrsIndCont` >0  or `HrsGroupInt` >0  or `HrsSelfEmp` >0  or `HrsFac` > 0)";
   ...
   switch ($type) {
      case 'grandtotal':
         ...
         $cols = "FORMAT(SUM(IFNULL(`dol_indComp`,0) + IFNULL(`dol_indCont`,0) + IFNULL(`dol_GroupInt`,0)  + IFNULL(`dol_SelfEmp`,0) + IFNULL(`dol_Facility`,0)) / count( DISTINCT `id`),2) avggrosswage";
         break;
         
         case 'averagehourly':
         $cols = "FORMAT(SUM(IFNULL(`dol_indComp`,0) + IFNULL(`dol_indCont`,0) + IFNULL(`dol_GroupInt`,0)  + IFNULL(`dol_SelfEmp`,0) + IFNULL(`dol_Facility`,0)) / SUM(IFNULL(TotalHours,0)),2) averagehourly";
         break;
         ...
         break;
         case 'coltotalhrs':
         $cols = "FORMAT(SUM(IFNULL(`HrsIndComp`,0) + IFNULL(`HrsIndCont`,0) + IFNULL(`HrsGroupInt`,0)  + IFNULL(`HrsSelfEmp`,0) + IFNULL(`HrsFac`,0)),2) totalhours";
         break;
         case 'colavghrs':
         $cols = "FORMAT(SUM(IFNULL(`HrsIndComp`,0) + IFNULL(`HrsIndCont`,0) + IFNULL(`HrsGroupInt`,0)  + IFNULL(`HrsSelfEmp`,0) + IFNULL(`HrsFac`,0)) / count(DISTINCT `id`),2) avghrs";
         break;
         case 'colnumover20':
         $cols = "FORMAT(SUM(IF((`HrsIndComp` AND 1 AND (`HrsIndComp` / 4) > 20) OR (`HrsIndCont` AND 1 AND (`HrsIndCont` / 4) > 20) OR (`HrsGroupInt` AND 1 AND (`HrsGroupInt` / 4) > 20) OR (`HrsSelfEmp` AND 1 AND (`HrsSelfEmp` / 4) > 20) OR (`HrsFac` AND 1 AND (`HrsFac` / 4) > 20), 1, 0)), 0) numover20";
         break;
         case 'colpercover20':
         $cols = "FORMAT(SUM(IF((`HrsIndComp` AND 1 AND (`HrsIndComp` / 4) > 20) OR (`HrsIndCont` AND 1 AND (`HrsIndCont` / 4) > 20) OR (`HrsGroupInt` AND 1 AND (`HrsGroupInt` / 4) > 20) OR (`HrsSelfEmp` AND 1 AND (`HrsSelfEmp` / 4) > 20) OR (`HrsFac` AND 1 AND (`HrsFac` / 4) > 20), 1, 0))  / count(DISTINCT `id`) * 100, 1) percover20";
         break;
         
            
            case 'colpercvol':
            $cols = "FORMAT(SUM(IF(`YN_vol` = 'Y', 1,0)) / count(DISTINCT `id`) * 100, 1) percvol";
            break;
      }
...
*/
	static function getRowData($report, $subreport, $level = 'provider') {
		$f		= mdda::getFilterValues();
		$totalhours = "IFNULL(`HrsIndComp`,0) + IFNULL(`HrsIndCont`,0) + IFNULL(`HrsGroupInt`,0) + IFNULL(`HrsSelfEmp`,0) + IFNULL(`HrsFac`,0)";
		if ($subreport != 'hours') { $hours = array('HrsIndComp','HrsIndCont','HrsSelfEmp', 'HrsGroupInt', 'HrsFac', 'HrsComNonWork', 'HrsFacNonWork'); } else { $hours = array('HrsIndComp','HrsIndCont','HrsSelfEmp', 'HrsGroupInt', 'HrsFac'); }
		if ($extra) {
		...
		if ($subreport == "number") {
			...
			$wages	= array('dol_indComp','dol_indCont', 'dol_GroupInt','dol_selfEmp', 'dol_Facility');
      ...
      elseif ($subreport == "pto") {
      			...
      			foreach ($wages as $key =>$val) {
                  $cols  .= ",\nFORMAT(REPLACE(SUM(IF(IFNULL(`YN_pto`,0) & $val = $val, 1, 0)), ',', '') / REPLACE(SUM(IF(IFNULL(`$key`,0) > 0, 1, 0)),',', '') * 100,2)";
		...
		if ($report == 'individual' && $f['region']) {
			$sql .= 'UNION ' . "SELECT '<rr/><strong>{$f['region']}</strong>', $cols FROM `spec_mdda6` WHERE `region` = '{$f['region']}' AND DATE_FORMAT(`Reporting_Period`,'%M %Y') = '{$f['year']}' GROUP BY `region` HAVING $having\n"; 
		}
		if ($report == 'individual') {
			$sql .= 'UNION ' . "SELECT '<zz/><strong>State</strong>', $cols FROM `spec_mdda6` WHERE `region` IS NOT NULL AND `region` <> '' AND DATE_FORMAT(`Reporting_Period`,'%M %Y') = '{$f['year']}' GROUP BY `reporting_period` HAVING $having\n"; 
		}
		print "<!-- $sql\n-->";
		$db = Database::getDatabase();
	...
	static function getYearSelect($name, $val) {
		...
		$db = Database::getDatabase();
		$rs = $db->query("SELECT DISTINCT DATE_FORMAT(`Reporting_Period`,'%M %Y') AS `y` FROM `spec_mdda6` order by `y` ");
		while ($row = $db->fetch_assoc($rs)) {
	...
	static function getReportingDates($year) {
      ...
      $modyear = substr($year, $yearlen,4);
		$db = Database::getDatabase();
		$rs = $db->query("SELECT CONCAT(`joined`.`start_date`,' to ', `joined`.`end_date`) from `mdda_reporting_period` joined WHERE `joined`.`end_date` LIKE '%$modyear'");
