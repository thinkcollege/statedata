...
*/
function getRegions($element_name, $showAll = 1, $provider = 'ALL') { 
	$database = Database::getDatabase();
	
	if ($provider == 'ALL'){
		$results = $database->query("Select distinct region from spec_dmr3 where region != 'x' order by region");
	} else {
		$results = $database->query("Select distinct region from spec_dmr3 where vendor = '$provider' order by region");
	}
...
function getProviders($element_name, $region, $showAll = 1) { 
	...
	if ($region == 'ALL' || $region=='0') {
		$query = "Select distinct vendor from spec_dmr3 order by vendor";
	} else {
		$query = "Select distinct vendor from spec_dmr3 where region='". $region ."' order by vendor";
	}
...
function getActivityVariables() { 
	...
	$html= $html . "<tr><td><input type=radio name=variable value='meanhourlywage'></td><td>Mean hourly wage for individual, group and facility employment</td></tr>\n";
...
function getRegionVariableArray ($variable, $year) {
	...
	for($i=0; $i<count($arRegions); $i++) {
		...
		if ($variable == 'Num_HrsInd') {
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsInd is not null and HrsInd != 0" . $whereClause;

			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Num_HrsGroup') {
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsGroup is not null and HrsGroup != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Num_HrsFac') {
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsFac is not null and HrsFac != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Num_HrsVolunteer') {
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Num_HrsTransition') {
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsTransition is not null and HrsTransition != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
			...
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' " . $whereClause;
			$results = $database->query($query);
			...
			$query = "Select avg(HrsInd) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsInd is not null and HrsInd != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
			
		} elseif ($variable == 'Avg_hrsGroup') {
			$query = "Select avg(hrsGroup) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and hrsGroup is not null and hrsGroup != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Avg_HrsFac') {
			$query = "Select avg(HrsFac) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsFac is not null and HrsFac != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Avg_HrsVolunteer') {
			$query = "Select avg(HrsVolunteer) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Avg_HrsTransition') {
			$query = "Select avg(HrsTransition) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsTransition is not null and HrsTransition != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');

		
		} elseif ($variable == 'Mean_HrsInd') {
			$query = "Select dol_ind/HrsInd as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsInd is not null and HrsInd != 0 and dol_ind is not null and dol_ind != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
			
		} elseif ($variable == 'Mean_hrsGroup') {
			$query = "Select dol_group/hrsGroup as total from spec_dmr3 where region='" . $arRegions[$i] . "' and hrsGroup is not null and hrsGroup != 0 and dol_group is not null and dol_group != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Mean_HrsFac') {
			$query = "Select  dol_facility/HrsFac as total from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsFac is not null and HrsFac != 0 and dol_facility is not null and dol_facility != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
			...
			$query = "Select sum(HrsInd) as part from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsInd is not null and HrsInd != 0" . $whereClause;

			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');
		
			$query = "Select sum(totalHours) as total from spec_dmr3 where region='" . $arRegions[$i] . "' " . $whereClause;
			...
			$query = "Select sum(HrsGroup) as part from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsGroup is not null and HrsGroup != 0" . $whereClause;
			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');
			...
			$query = "Select sum(HrsFac) as part from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsFac is not null and HrsFac != 0" . $whereClause;
			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');
			...
			$query = "Select sum(HrsVolunteer) as part from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsVolunteer is not null and HrsVolunteer != 0" . $whereClause;
			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');
			...
			$query = "Select sum(HrsTransition) as part from spec_dmr3 where region='" . $arRegions[$i] . "' and HrsTransition is not null and HrsTransition != 0" . $whereClause;
			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');
			...
			$query = "Select avg(dol_ind) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and dol_ind is not null  and dol_ind != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Avg_dol_group') {
			$query = "Select avg(dol_group) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and dol_group is not null  and dol_group != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');

		} elseif ($variable == 'Avg_dol_Facility') {
			$query = "Select avg(dol_Facility) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and dol_Facility is not null  and dol_Facility != 0" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
			
		} elseif ($variable == 'Per_YNInd') {
			$query = "Select count(*) as ys from spec_dmr3 where region='" . $arRegions[$i] . "' and trim(YNInd) = 'Y'" . $whereClause;
			$results = $database->query($query);
			$ys = $database->fetch_result($results,$result,'ys');

			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and ( trim(YNInd) in ('Y','N')) " . $whereClause;
			$results = $database->query($query);
			$total = $database->fetch_result($results,$result,'total');
			...
			$query = "Select count(*) as ys from spec_dmr3 where region='" . $arRegions[$i] . "' and trim(YNGroup) = 'Y'" . $whereClause;
			$results = $database->query($query);
			$ys = $database->fetch_result($results,$result,'ys');

			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "'and ( trim(YNGroup) in ('Y','N')) " . $whereClause;
			$results = $database->query($query);
			$total = $database->fetch_result($results,$result,'total');
			...
			$query = "Select count(*) as ys from spec_dmr3 where region='" . $arRegions[$i] . "' and trim(YNFacility) = 'Y'" . $whereClause;
			$results = $database->query($query);
			$ys = $database->fetch_result($results,$result,'ys');

			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and ( trim(YNFacility) in ('Y','N'))" . $whereClause;
			$results = $database->query($query);
			$total = $database->fetch_result($results,$result,'total');
			...
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and trim(IndSupEmp) = 'Y'" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i]  = $database->fetch_result($results,$result,'total');
		} elseif ($variable == 'Num_GroupSupEmp') {
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and trim(GroupSupEmp) = 'Y'" . $whereClause;
			$results = $database->query($query);
			$returnArray[$i]  = $database->fetch_result($results,$result,'total');
			...
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and ( trim(IndSupEmp) in ('Y','N'))" . $whereClause;
			$results = $database->query($query);
			$total  = $database->fetch_result($results,$result,'total');
			...
			$query = "Select count(*) as total from spec_dmr3 where region='" . $arRegions[$i] . "' and ( trim(GroupSupEmp) in ('Y', 'N')) " . $whereClause;
			$results = $database->query($query);
			$total  = $database->fetch_result($results,$result,'total');
...
function getActivityVariableArray ($variableType, $region, $provider, $year = '04') {
	...
	$results = $database->query("Select count(*) as total from spec_dmr3 where 1=1" . $whereClause );
	//echo "Select count(*) as total from spec_dmr3 where 1=1" . $whereClause ."<BR>" .  $variableType ;
	$totalRecords = $database->fetch_result($results,$result,'total');
			
	if ($variableType == 'numberinactivity') {
		for($i=0; $i<count($columArray); $i++) {
			$query = "Select count(*) as total from spec_dmr3 where " . $columArray[$i] . " is not null and ". $columArray[$i] . " !=0 ". $whereClause ;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
	} elseif ($variableType == 'percent') {
		for($i=0; $i<count($columArray); $i++) {
			$results = $database->query("Select count(*) as total from spec_dmr3 where " . $columArray[$i] . " is not null and ". $columArray[$i] . " !=0  ". $whereClause);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columArray); $i++) {
			$query = "Select sum(" . $columArray[$i] . ") as total from spec_dmr3 where " . $columArray[$i] . " is not null and ". $columArray[$i] . " !=0  " . $whereClause;
//echo $query ." <BR>";
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columArray); $i++) {
			...
			//$query = "CREATE TEMPORARY TABLE medians SELECT x." . $columArray[$i] . " medians FROM spec_dmr3 x, spec_dmr3 y	GROUP BY x." . $columArray[$i] . "	HAVING SUM(y." . $columArray[$i] . " <= x." . $columArray[$i] . ") >= COUNT(*)/2 AND SUM(y." . $columArray[$i] . " >= x." . $columArray[$i] . ") >= COUNT(*)/2;";
	
			//$temp_table_q = $database->query($query);
			
			//$query = "SELECT AVG(medians) AS median FROM medians";
			$query = "SELECT AVG(" . $columArray[$i] . ") AS mean FROM spec_dmr3 where  " . $columArray[$i] . " is not null and ". $columArray[$i] . " !=0  " . $whereClause;
			$results = $database->query($query);
...
// totalwages, ALL, ALL, 04
		...
		//$query = "Select count(*) as total from spec_dmr3 where 1=1 " . $columWagesArray[$i] . " is not null  ". $whereClause;
		//$results = $database->query($query);
		//$totalRecords = $database->fetch_result($results,$result,'total');

		for($i=0; $i<count($columWagesArray); $i++) {
		$query = "Select sum(" . $columWagesArray[$i] . ") as total from spec_dmr3 where " . $columWagesArray[$i] . " is not null and ". $columWagesArray[$i] . " !=0  ". $whereClause;
	//	echo $query ."<BR>";
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$results = $database->query("Select avg(" . $columWagesArray[$i] . ") as total from spec_dmr3 where " . $columWagesArray[$i] . " is not null and ". $columWagesArray[$i] . " !=0  " . $whereClause);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$query = "Select count(" . $columWagesArray[$i] . ") as total from spec_dmr3 where trim(" . $columWagesArray[$i] . ") = 'Y' ". $whereClause;
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$query = "Select count(*) as total from spec_dmr3 where trim( " . $columWagesArray[$i] . " ) IN ('N', 'Y') ". $whereClause;
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$query = "Select " . $columWagesArray[$i] . "/" . $columHoursArray[$i] . " as total from spec_dmr3 where " . $columWagesArray[$i] . " is not null and ". $columWagesArray[$i] . " !=0  and " . $columHoursArray[$i] . " is not null and ". $columHoursArray[$i] . " !=0  " . $whereClause;
			$results = $database->query($query);
			$returnArray[$i] = $database->fetch_result($results,$result,'total');
		}
	} 
...
function getRowData($provider, $report, $region, $year ='none', $showregion = 0) {
	...
	$query = "Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' " . $region;
//	echo $query;
	$results = $database->query($query);
	$total = $database->fetch_result($results,$result,'total');
	echo "<td>" .$total ."</td>\n";
	if ($report == "number") {
		...
		$query = "Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and HrsInd is not null and HrsInd != 0" . $region;
		$results = $database->query($query);
		$HrsInd = $database->fetch_result($results,$result,'total');
		echo "<td>" . $HrsInd ."</td>\n";
	
		$query = "Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and HrsGroup is not null and HrsGroup != 0" . $region;
		$results = $database->query($query);
		$HrsGroup = $database->fetch_result($results,$result,'total');
		echo "<td>" . $HrsGroup ."</td>\n";
	
		$query = "Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and HrsFac is not null and HrsFac != 0" . $region;
		$results = $database->query($query);
		$HrsFac = $database->fetch_result($results,$result,'total');
		echo "<td>" . $HrsFac ."</td>\n";
	
		$query = "Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and (HrsVolunteer > 0  )" . $region;
		$results = $database->query($query);
		$HrsVolunteer = $database->fetch_result($results,$result,'total');
		echo "<td>" . $HrsVolunteer ."</td>\n";
	
		$query = "Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and HrsTransition is not null and HrsTransition != 0" . $region;
		$results = $database->query($query);
		...
		for($i=0; $i<count($columArray); $i++) {
			$query = "SELECT AVG(" . $columArray[$i] . ") AS mean FROM spec_dmr3 where Vendor='" . $provider . "'  and (" . $columArray[$i] . " > 0 or " . $columArray[$i] . " = 0) " . $region;
			$results = $database->query($query);
			echo "<td>" . number_format($database->fetch_result($results,$result,'mean'), 1, '.', '') . "</td>\n";
		}
		for($i=0; $i<count($columArray); $i++) {
			$query = "Select sum(" . $columArray[$i] . ") as part from spec_dmr3 where Vendor='" . $provider . "' and " . $columArray[$i] . " > 0" . $region;

			$results = $database->query($query);
			$part = $database->fetch_result($results,$result,'part');
		
			$query = "Select sum(totalHours) as total from spec_dmr3 where Vendor='" . $provider . "'  and (" . $columArray[$i] . " > 0 or " . $columArray[$i] . " = 0) " . $region;
		...
		for($i=0; $i<count($columWagesArray); $i++) {
			$results = $database->query("Select avg(" . $columWagesArray[$i] . ") as total from spec_dmr3 where Vendor='" . $provider . "' and " . $columWagesArray[$i] . " is not null and ". $columWagesArray[$i] . " !=0  " . $region);
		...
		for($i=0; $i<count($columWagesOverArray); $i++) {
			$results = $database->query("Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and trim(" . $columWagesOverArray[$i] . ") in ('N','Y') " . $region);
			$totalRecords =  $database->fetch_result($results,$result,'total') ;
		
			$results = $database->query("Select count(*) as total from spec_dmr3 where Vendor='" . $provider . "' and trim(" . $columWagesOverArray[$i] . ") ='Y'" . $region);
