...
*/
	static function getRegions($element_name, $selected = '', $showAll = 1, $provider = 0, $regiontype = 'normal', $year = 0) {
		...
		$html = $showAll == 1 && $db->num_rows($rs) > 1 ? '<option value="">All Regions</option>' : '';
	...
	static function getRegionArrayById($provider_id) {
		$db	 = Database::getDatabase();
		$ret = array();
		$rs  = $db->query("SELECT distinct region from spec_dmr6 where vendor_id = $provider_id and TRIM(`region`) order by region");
		while ($row = $db->fetch_assoc($rs)){
			$ret[] = $row['region'];
		}
		return $ret;
...
*/
	static function getRowData($report, $subreport, $level = 'provider') {
		...
		$sql = "SELECT CONCAT('<strong>', `vendor`, '</strong>') AS `grouping`, $cols FROM `spec_dmr6` WHERE `region` IS NOT NULL AND `region` <> '' $where GROUP BY `vendor_id` HAVING $having\n";
		if ($report == 'individual' && $f['region']) {
			$sql .= 'UNION ' . "SELECT '<strong>{$f['region']}</strong>', $cols FROM `spec_dmr6` WHERE `region` = '{$f['region']}' AND `reporting_period` = {$f['year']} GROUP BY `region` HAVING $having\n"; 
		}
		if ($report == 'individual') {
			$sql .= 'UNION ' . "SELECT '<strong>State</strong>', $cols FROM `spec_dmr6` WHERE `region` IS NOT NULL AND `region` <> '' AND `reporting_period` = {$f['year']} GROUP BY `reporting_period` HAVING $having\n"; 
		}
		print "<!-- $sql\n-->";
		$db = Database::getDatabase();
	...
	static function getYearSelect($name, $val) {
		...
		$db = Database::getDatabase();
		$rs = $db->query('SELECT DISTINCT `reporting_period` AS `y` FROM `spec_dmr6`');
		while ($row = $db->fetch_assoc($rs)) {
