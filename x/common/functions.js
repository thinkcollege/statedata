...
function init_trend (){
	...
	maxNumber = 1;
	updateYearDisplay($("#reporttype").val());
	
	$("#reporttype").change(function () {
		updateYearDisplay($(this).val());
	});

	show_hierarchy_parent();
	
	
	//todo
	//expand selected activity category if ine is selected
	
	$("input[@name=rYear]").click(function () {
		var selectedItem = $("input[@name=rYear]:checked").val();
		$("#year").val(selectedItem);
		return true;
	});

	$("input[@name=cbYear]").click(function () {
		var selectedItem = [];
		$("input[@name=cbYear]:checked").each(function(){
			selectedItem[selectedItem.length] = $(this).val();
		});
		$("#year").val(selectedItem.join(","));
		return true;
	});
	
	$("input[@name=subcat]").click(function () {
		var selectedItem = $("input[@name=subcat]:checked").val().split("_");
		get_states(selectedItem[0]);
		return true;
	});

	$("#trendForm").submit(function() {
		var selectedStates = "'xxx'";
		$("select#states option:selected").each(function () {
			var opt = $(this);
			selectedStates = selectedStates + ",'" + opt.val() +"'";
			
		});
		$("#selectedstates").val(selectedStates);
		
		if($("#year").val() == ""){
			alert("Please select a year");
			return false;
		}
		if(!$("input[@name=subcat]").is(":checked")){
			alert("Please select an activity");
			return false;
		}
		if(selectedStates == "'xxx'"){
...
function updateYearDisplay(reportType){
		switch(reportType)
		{
		case "single":
		  maxNumber = 1;
			$("#statesLabel").html("Select a program for your report:");
			$("select#states option:selected").each(function () {
				var opt = $(this);
				opt[0].selected = false;
			});
			
			if($("#yearPlaceholderRadio").is(':visible')){
				$("#year").val("");
			}
			$("#yearPlaceholderRadio").hide();
			$("#yearPlaceholderCheckbox").show();
			
			$("input[@name=rYear]:checked").each(function(){
		...
		case "multi":
			maxNumber = 3;
			$("#statesLabel").html("Select up to 3 programs for your report: (To select more than one item, Ctrl-Click or Cmd-Click on Mac)");
		...
		case "oneyear":
		  maxNumber = 2;
			$("#statesLabel").html("Select up to 2 programs for your report: (To select more than one item, Ctrl-Click or Cmd-Click on Mac)");
			$("select#states option:selected").each(function () {
				var opt = $(this);
				opt[0].selected = false;
			});
			$("#yearPlaceholderRadio").show();
			$("#yearPlaceholderCheckbox").hide();
...
function init_activity_summary() {
	//bind on chang to activity
		maxNumber = 3;

		if ($("select#states option").length > 0){
			$("#statePlaceholder").show();
		}		
		$("#activity").change(function () {
			var selectedActivity = $(this).val();
			get_states(selectedActivity);
...
function get_states(selectedActivity){
	...
	//		debugger;
		...
		success: function (response){
				var selectedOptions = $("select#states option:selected");
				...
				for (var i = 0; i < response.length; i++) {
					...
					selectedOptions.each(function () {
						var opt = $(this);
						if (opt.text() == response[i].optionDisplay) {
							selected = " selected=\"selected\" ";
						}
					});
					options += '<option value="' + response[i].optionValue + '" '+ selected +'>' + response[i].optionDisplay + '</option>';
				}
...
function init_overview_hierarchy(){
	$(".expandImage").click(function(){
		var img = $(this);
		var id = $(this)[0].id;
		var containerId = id.replace("exp","cont");
		
		if (img.hasClass("opened")){
			img.removeClass("opened");
			$(this)[0].src = "../images/plus.gif";
			$("#" + containerId + " > div").hide();
		} else {
			img.addClass("opened");
			$(this)[0].src = "../images/minus.gif";
			$("#" + containerId + " > div").show();
		}
	});
}
function init_activity_hierarchy(){
	$(".expandImage").click(function(){
		var img = $(this);
		var id = $(this)[0].id;
		var containerId = id.replace("exp","cont");
...
function show_hierarchy_parent(){
	var subcatObj = $("input[@name=subcat]:checked");
	
	if (subcatObj.length > 0 ){
		//subcatObj.parents(".activityContainer").show();
		//subcatObj.parents(".activityContainer").each(function (){
		//	var div = $(this).parent().;
		//	
		//for 'activityContainer' type of house
		//});
		//to do, find img chilren fo each selected it with class 
		var id = subcatObj[0].id;
		var containerId = "cont_" + subcatObj.val();
		var imgId = "exp_" + subcatObj.val();
		
		var containerId_alt = id.replace("subcat","cont");
		var imgId_alt = id.replace("subcat","exp");
		($("#" + containerId + " > table").length == 0)? $("#" + containerId_alt + " > table").show() : $("#" + containerId + " > table").show()
		
		var img = $("#" + imgId);
		var img_alt = $("#" + imgId_alt);
		if (img.length ==0 ) {
			img_alt.addClass("opened");
			img_alt[0].src = "../images/minus.gif";
		} else {
			img.addClass("opened");
			img[0].src = "../images/minus.gif";
		}
		var selectedItem = subcatObj.val().split("_");
		get_states(selectedItem[0]);
		
	}
}

function chech_vars(elm, formName,elementName, total_vars) {
	var m;
	frm = eval("document." + formName );
	checkArray = eval("document." + formName + "." + elementName );

	variables = 0;	
	showAlert = false;

	frm.variable1.value = 0;
	if (total_vars == 2) {
		frm.variable2.value = 0;
	}

	if (checkArray.length && checkArray.checked) {
		variable = eval("document." + formName + ".variable1");
		variable.value = checkArray.value;

	} else {

		for(m=0; m<checkArray.length; m++) {
			if(checkArray[m].checked) {
				variables++;
				
				if (total_vars ==2) {
					if ( variables == 1 || variables == 2) {
						variable = eval("document." + formName + ".variable" + variables);
						variable.value = checkArray[m].value;
					} else if (variables >2 )  {
						showAlert = true;
						elm.checked = false;
	
					}
				} else if (total_vars ==1) {
					if ( variables == 1 ) {
						variable = eval("document." + formName + ".variable" + variables);
						variable.value = checkArray[m].value;
					} else if (variables >1 )  {
	...
	if (showAlert) {
		if (total_vars == 2) {
			alert ("You may only select up to two variables");
		} else {
			alert ("You may only select one variable");
		}
...
/**
 * Check if value is a number (decimals ok)
 */
...
{
	var arrDaysInMonth=new Array(0,31,28,31,30,31,30,31,31,30,31,30,31);
	//split the date into 3parts
	dtValue=dtValue.split("/")
	if (dtValue.length!=3)
		return false;
	var tempMonth = dtValue[0];
	var Month = parseInt(dtValue[0]);
	if (tempMonth.length ==2 && tempMonth.charAt(0) == "0") {
		Month = parseInt(tempMonth.charAt(1));
	}
	...
	if(isLeapYear(Year))
		arrDaysInMonth[2]=29;
	//validate
	if(Month<1 || Month>12 || Day<1 || Day>arrDaysInMonth[Month])
		return false;
	Month -= 1;
