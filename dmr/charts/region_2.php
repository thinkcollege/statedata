...
$template->add_region('content','
	<?php
		...
		$arVariableValues = $dmr->getRegionVariableArray ($variable, $year);
		$arColumnNames = $dmr->getRegionColumnNamesArray ($year);
		$axis = $dmr->getRegionAxisLabel($variable);
		$legend = $dmr->getRegionLegendName($variable, $year);
		...
		for($i=0; $i<count($arColumnNames); $i++) {
			if (substr_count($arColumnNames[$i], "x_") > 0 ) {
				$arColumnNames[$i] = substr($arColumnNames[$i],2);
			}
		...
		//echo "<br>" . $pcScript ;
		echo "<!-- internal:$mychart->internalCommPortAddress exteral:$mychart->externalServerAddress -->";
		if ($print == 0) {
		...
		if ($print == 0) {
			echo "<br><a target=\'_new\'  href=\'region_2.php?print=1&year=$year&variable=$variable\' >Printer-Friendly Format</a>";
		}
		#echo "<br> $axis_note";
